...
class PointNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feature = normalize_point_cloud(feature)
        # TODO: rotation to feature

        if self._train == 0:
            # 绕z轴旋转
            theta = np.random.uniform(0, np.pi * 2)
            feature = rotate_around_z(feature, theta)
            # 应该先旋转, 再jitter, 这样不会削弱旋转的作用,但是猜测是差别不大
            # jitter
...
if __name__ == "__main__":
    datapath = '/home/steven/code/cloud_point/pointnet/data/modelnet40_normal_resampled'
    train_data = PointNetDataset(datapath, train=0)
